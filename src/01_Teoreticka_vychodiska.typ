...
Za účelem porozumění tématu nyní vyjmenujeme definice a teoretická východiska pro tuto práci.
...
Funkční analýza se zaměřuje na identifikaci funkcí, které systém poskytuje, bez ohledu na jejich technickou implementaci.
Jejím výsledkem je přehled jednotlivých funkcionalit systému, rozdělených podle logických celků.
@Sommerville2015

Tato analýza umožňuje strukturovaně popsat klíčové moduly systému a vazby mezi nimi.
Na základě identifikovaných datových toků určit technické a procesní závislosti.
...
Je možné využít grafického znázornění stávajícího stavu pomocí diagramů.
Diagram toku dat Data Flow Diagram - DFD, zobrazuje tok dat, procesy a úložiště.
Kontextový diagram, vymezuje hranice systému a jeho interakce s okolním prostředím.
@Sommerville2015
...
Funkční požadavky popisují konkrétní činnosti a služby, které má IS poskytovat.
Například zobrazení aktuálních poloh vozidel.
Nefunkční požadavky stanovují omezení a kvalitativní kritéria systému.
Například požadavky na výkon, dostupnost, bezpečnost nebo použitelnost.
...
Finální návrh budoucího stavu zahrnuje poskytované funkce, způsoby, jakým tyto funkce budou podporovat podnikové procesy, včetně toků dat a úložišť nového IS.
@isManagement
...
Slouží k porozumění fungování organizace prostřednictvím jejích podnikových procesů.
Na podnik je možné nahlížet jako soustavu vzájemně propojených procesů, které vytvářejí hodnotu pro zákazníka a podporují naplnění strategických cílů organizace.
V této analýze je kladen důraz na přesnou identifikaci částí IS, které mají přímý dopad na chod organizace.
Cílem je pochopit tok činností, vstupy, výstupy a odpovědnosti.
@Dumas2018
...
Čtvrtým krokem je *návrh optimalizace (To-Be)*.
Na základě zjištěných nedostatků dojde k návrhu nových procesů po jejich optimalizaci.
Zohledňují se možnosti automatizace, digitalizace a podpory IS.
...
Implementace může zahrnovat zavedení nového informačního systému nebo úpravy stávajícího řešení tak, aby podporovalo optimalizované procesy.
...
Pro analýzu IS je vhodné využít několik typů diagramů.
*Typy BPMN diagramů a jejich význam*:
...
@weske2024bpm
*Kontextové diagramy (Collaboration / Choreography Diagrams)* znázorňují interakci mezi externími aktéry a systémem.
Jsou vhodné pro vymezení hranic systému a zobrazení komunikačních toků.
@omg2013bpmn202
...
Při *návrhu budoucího stavu (To-Be)* dokáže vizualizovat procesy po implementaci nového IS.
